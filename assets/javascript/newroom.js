...
roomName.onblur = () => {
  if (roomName.value === "") nameEnLabel.classList.remove("active-label");
};
...
roomPrice.onblur = () => {
  if (roomPrice.value === "") roomPriceLabel.classList.remove("active-label");
};
...
guestInput.onblur = () => {
  if (guestInput.value === "") guestLabel.classList.remove("active-label");
};
...
roomNameAr.onblur = () => {
  if (roomNameAr.value === "") roomNameLabel.classList.remove("active-label");
};
...
subTitle.onblur = () => {
  if (subTitle.value === "") subTitleLabelEn.classList.remove("active-label");
};
...
floor.onblur = () => {
  if (floor.value === "") floorLabel.classList.remove("active-label");
};
...
createRoomBtn.addEventListener("click", async function (e) {
  e.preventDefault();
  ...
  formData.append("room_services", servicesVal);

  await fetch("https://mountain.lavetro-agency.com/api/dashboard/rooms", {
    method: "POST",
    headers: {
